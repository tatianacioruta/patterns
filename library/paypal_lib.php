...
class paypal_lib {
	...
	{
		$str = '';
		$str .= '<form id="paypal_form" method="post" action="'.$this->paypal_url.'" name="'.$form_name.'"/>' . "\n";
	...
	function validate_ipn()
	{
		...
		$ipnURL = ($url_parsed['scheme'] == "https") ? "ssl://" . $url_parsed['host'] : $url_parsed['host'];
		// Likewise, if using ssl, then need to change port from 80 to 443
		$ipnPort = ($url_parsed['scheme'] == "https") ? 443 : 80;

		// open the connection to paypal
		$fp = fsockopen($ipnURL, $ipnPort, $err_num, $err_str,30);
		if(!$fp)
		{
	...
	function curlPost($paypalreturnarr)
	{

		$paypalreturnarr['cmd'] = "_notify-validate";
		$postdata = http_build_query($paypalreturnarr);
